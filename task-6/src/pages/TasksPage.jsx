...
//  Fetches from MySQL via Express API (with Auth)
// =============================================
...
export default function TasksPage() {
  ...
  const handleAdd = async (e) => {
    ...
    try {
      const newTask = await createTask({ text: newText.trim(), priority: newPri })
  ...
  const handleSaveEdit = async (id) => {
    ...
    try {
      const updated = await updateTask(id, { text: editText.trim(), priority: editPri })
      setTasks(prev => prev.map(t => t.id === id ? updated : t))
      setEditId(null)
    ...
    <>
      ...
      <div>
        <div className={styles.pageHead}>
          ...
          <button className={styles.refreshBtn} onClick={loadTasks}>↺ Refresh</button>
        </div>
